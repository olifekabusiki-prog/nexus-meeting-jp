...
function SessionContent() {
    ...
    return (
      <div className="h-dvh bg-mtg-black flex flex-col items-center justify-center gap-6 px-8">
        <div className="text-center space-y-2">
          <div className="text-white font-semibold tracking-wider">音声認識エンジンを準備中</div>
          <div className="text-mtg-mid text-xs">初回のみ数十秒かかります</div>
        </div>
        <div className="w-full max-w-xs bg-mtg-surface rounded-full h-1.5">
          <div
            className="bg-mtg-red h-1.5 rounded-full transition-all duration-300"
  ...
  return (
    <div className="h-dvh bg-mtg-black flex flex-col overflow-hidden">
      ...
      {/* トランスクリプトエリア */}
      <div ref={scrollRef} className="flex-1 overflow-y-auto no-scrollbar px-5 py-4 space-y-3 min-h-0">
          ...
          <div className="text-center mt-20 space-y-2">
            {isCapturing ? (
              <>
                ...
                <div className="text-mtg-border text-xs">チャンクごとに文字が表示されます（約4秒ごと）</div>
              </>
            ) : (
              <>
        ...
        {transcript.map((entry) => {
          const style = SPEAKER_STYLES[entry.speaker_role] ?? SPEAKER_STYLES.unknown
          ...
          return (
            <div key={entry.id} className={`flex flex-col fade-up ${isRight ? 'items-end' : 'items-start'}`}>
              ...
              <div className={`max-w-[78%] rounded-2xl px-4 py-2.5 ${
                isRight
                  ? 'bg-mtg-red-dim border border-mtg-red/30 rounded-tr-sm'
                  : 'bg-mtg-surface border border-mtg-border rounded-tl-sm'
              }`}>
      ...
      <div className="shrink-0 border-t border-mtg-border px-5 py-5">
        <div className="flex items-center justify-center gap-6 max-w-sm mx-auto">
          ...
          <button
            ...
            className="flex flex-col items-center gap-1.5 text-mtg-mid disabled:opacity-30"
          >
          ...
          <div className="flex flex-col items-center gap-1.5 w-12">
            <div className="w-12 h-12 rounded-full border border-mtg-border flex items-center justify-center">
              <span className="text-mtg-light text-xs font-mono font-bold">
                {transcript.length}
              </span>
            </div>
            <span className="text-mtg-mid text-xs">発言数</span>
          </div>
...
export default function SessionPage() {
  return (
    <Suspense fallback={
      <div className="h-dvh bg-mtg-black flex items-center justify-center">
        <p className="text-mtg-mid text-sm">読み込み中...</p>
      </div>
    }>
      <SessionContent />
    </Suspense>
